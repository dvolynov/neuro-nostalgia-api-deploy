...
def retroify(
    ...
    os.makedirs("api/v2/cache", exist_ok=True)

    with open(f"api/v2/cache/{page_id}.html", "w") as file:
        file.write(html)

    base_url = f"{request.url.scheme}://{request.url.hostname}"
    return {
        "url": f"{base_url}/api/v2/webpage/{page_id}",
        "css": css
    }
...
async def webpage(page_id: str):
    file_path = f"api/v2/cache/{page_id}.html"
